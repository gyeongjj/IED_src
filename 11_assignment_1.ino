...
float duty_continuous;// servo continuous position (0~180 degree)
...
// get a distance reading from the USS
  ...
  duty_continuous = _DUTY_MIN + ((_DUTY_MAX - _DUTY_MIN)/(dist_max - dist_min)) * (dist_raw - dist_min);
...
// adjust servo position according to the USS read value
  ...
  if (dist_raw<=dist_min){
    ...
    myservo.writeMicroseconds(duty_continuous);
  }
  else{
